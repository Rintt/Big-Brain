...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "opencode-run", "--agent", "opencode", "--prompt", "write a file"],
    { cwd, env: { ...env, BB_FAKE_DOCKER_LOG: dockerLogPath, OPENAI_API_KEY: "test-key" } }
  );
...
test("bb run --agent opencode requires OPENAI_API_KEY before creating artifacts", async () => {
  ...
  await assert.rejects(
    execFileAsync(process.execPath, [path.resolve("dist/cli/index.js"), "run", "--name", "missing-key", "--agent", "opencode", "--prompt", "write a file"], {
      cwd,
      env
    }),
    (error: unknown) => {
      assert.match(String((error as { stderr?: string }).stderr), /OPENAI_API_KEY is required/i);
      return true;
    }
  );
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "file-key", "--agent", "opencode", "--prompt", "write a poem"],
    { cwd, env: { ...env, BB_FAKE_API_KEY_LOG: apiKeyLogPath } }
  );
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "final-output", "--agent", "opencode", "--prompt", "write a poem"],
    { cwd, env: { ...env, OPENAI_API_KEY: "test-key" } }
  );

  const result = JSON.parse(await readFile(path.join(cwd, ".big-brain", "runs", "final-output", "result.json"), "utf8")) as { finalOutput?: string | null };
...
test("bb run requires exactly one of --agent or --agent-command", async () => {
  for (const args of [
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.match(String((error as { stderr?: string }).stderr), /exactly one of --agent or --agent-command/i);
        return true;
      }
    );
  }
...
test("bb run rejects unsupported agent values before creating run artifacts", async () => {
  ...
  await assert.rejects(
    execFileAsync(process.execPath, [path.resolve("dist/cli/index.js"), "run", "--name", "bad-agent", "--agent", "claude", "--prompt", "write a file"], { cwd }),
    (error: unknown) => {
      assert.match(String((error as { stderr?: string }).stderr), /unsupported agent/i);
      return true;
    }
  );
  await assert.rejects(readFile(path.join(cwd, ".big-brain", "runs"), "utf8"), { code: "ENOENT" });
...
`);
  ...
  await assert.rejects(
    execFileAsync(process.execPath, [path.resolve("dist/cli/index.js"), "run", "--name", "missing-opencode", "--agent", "opencode", "--prompt", "write a file"], {
      cwd,
      env: { ...env, OPENAI_API_KEY: "test-key" }
    }),
    (error: unknown) => {
      assert.match(String((error as { stderr?: string }).stderr), /OpenCode.*Docker image.*installed/i);
      return true;
    }
  );
});

async function fakeDockerEnv(cwd: string, script: string): Promise<NodeJS.ProcessEnv> {
