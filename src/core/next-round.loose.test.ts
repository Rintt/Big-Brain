...
test("package root exports run and opencode", async () => {
  const root = (await import(pathToFileURL(path.join(process.cwd(), "dist", "index.js")).href)) as {
    run?: unknown;
    opencode?: unknown;
  };
...
test("docker sandbox export describes the default Docker Sandbox", async () => {
  const sandbox = (await import(pathToFileURL(path.join(process.cwd(), "dist", "sandboxes", "docker.js")).href)) as {
    docker?: unknown;
  };
...
`,
  ...
  const resultJson = JSON.parse(await readFile(path.join(cwd, ".big-brain", "runs", "clean-branch", "result.json"), "utf8")) as {
    status?: string;
    events?: Array<{ type?: string; worktreePath?: string }>;
  };
...
`,
  ...
  const resultJson = JSON.parse(await readFile(path.join(cwd, ".big-brain", "runs", "dirty-branch", "result.json"), "utf8")) as {
    status?: string;
