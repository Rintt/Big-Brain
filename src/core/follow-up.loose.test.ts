...
test("run() rejects a missing name before creating run artifacts", async () => {
  ...
  await assert.rejects(run({ cwd, prompt: "write a file" } as Parameters<typeof run>[0]), /name/i);
...
`);
  ...
  try {
    ...
    await assert.rejects(
      run({ cwd, name: "missing-prompt", agentCommand: "node -e ''" } as Parameters<typeof run>[0]),
      /prompt/i
    );
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "docker-shell", "--agent-command", "node -e 'console.log(1)'", "--prompt", "write a file"],
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "default-network", "--agent-command", "node -e ''", "--prompt", "write a file"],
    { cwd, env: { ...env, BB_FAKE_DOCKER_LOG: dockerLogPath } }
  );
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "metadata", "--agent-command", "node -e ''", "--prompt", "write a file"],
    { cwd, env }
  );

  const result = JSON.parse(await readFile(path.join(cwd, ".big-brain", "runs", "metadata", "result.json"), "utf8")) as {
    status?: string;
    startedAt?: string;
    completedAt?: string;
  };
...
`);
  ...
  await execFileAsync(
    process.execPath,
    [path.resolve("dist/cli/index.js"), "run", "--name", "log-only", "--agent-command", "node -e ''", "--prompt", "secret prompt text"],
    { cwd, env }
  );
