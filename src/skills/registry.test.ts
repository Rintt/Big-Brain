...
async function makeRepoWithSkill(options: { folderName: string; skill: string }): Promise<string> {
  const cwd = await mkdtemp(path.join(tmpdir(), "big-brain-skills-"));
  await makeSkill(cwd, options.folderName, options.skill);
  return cwd;
}

async function makeSkill(cwd: string, folderName: string, skill: string): Promise<void> {
